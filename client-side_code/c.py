...
def is_port_open(ip, port=80, timeout=3):
    try:
        # 创建TCP socket并尝试连接
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            s.settimeout(timeout)
            result = s.connect_ex((ip, port))  # 0表示连接成功
            return result == 0
    except Exception:
        return False
        ...
        for line in json1.iter_lines():
            ...
            for z in data:
                if isinstance(z, dict):
                    ...
                    print(f"用户名：{base64_to_chinese(z["name"])}", end='  ')
        ...
        if input1 == '1':#读取密码
           Get()
            ...
            if is_port_open(device_ip):
                ...
                raise ConnectionFailed
            while True:
               user_site = input("网站/应用(T/退出)：").strip()
               if user_site == "":
                   print("请输入正确的网站/应用")
                   ...
                   break
            while True:
               user_name = input("用户名(T/退出)：").strip()
               if user_name == "":
                   print("请输入正确的用户名")
            ...
            ac["nums"] = f"{int(num) - 1}"
            name = input("用户名(不修改则直接回车(Enter))：").strip()
